...
local tasks = {
    ...
    { metalType = "STEEL",      itemType = "FULL_HELM",  itemLevel = 0, amount = 280 },
...
while API.Read_LoopyLoop() do
    ...
    if API.Compare2874Status(12) then
        print("[DEBUG] Compare2874Status 12 - likely finished smithing but waiting for interface to update - checking for options")
        o = hasOption()
        if o and o == "Continue making a burial item?" or
            ...
            print("[DEBUG] ", t and #t > 0 and t[1].textids or "No text found")
            if t and #t > 0 and t[1].textids and #t[1].textids > 0 then
                print("[DEBUG] Found text on interface after smithing - checking if it's the finish text")
                local text = t[1].textids
                if string.find(text, "You finish smithing") then
                    print("[DEBUG] Finished smithing item - clicking continue")
                    print("[DEBUG] DEBUG_USE_INTERFACE_OVER_KEYPRESS", DEBUG_USE_INTERFACE_OVER_KEYPRESS)
                    if DEBUG_USE_INTERFACE_OVER_KEYPRESS then
