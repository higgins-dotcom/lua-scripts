--[[
====================================================================================================
Bank Standers Manufacturing Script
====================================================================================================
Version: 1.0
Author: Higgins
Description: Automatically manufactures bank standers using Inventor's workbench with banking support

Recommended Location: Fort Forinthry

Requirements:
- Access to Bank chest and Inventor's workbench
- Materials for bank stander manufacturing
- Last bank preset configured with required materials

How it works:
1. Loads last bank preset from Bank chest
2. Checks for adamant bar (item 2361) in inventory
3. Uses Inventor's workbench to manufacture bank standers
...
local ITEM_IDS = {
	ADAMANT_BAR = 2361,
	BANK_STANDER_COMPLETE = 8847,
}

local VB_SETTINGS = {
	PRODUCT = 1170,
	CATEGORY = 1169,
	BANK_STANDER_ID = 37705,
	CATEGORY_ID = 6379,
...
local INTERFACE_IDS = {
	...
	MISC_SUPPORT_OPTION = { component = 1477, subcomponent = 916, option = 21 },
}

-- Utility Functions
local function hasAdamantBar()
	return Inventory:Contains(ITEM_IDS.ADAMANT_BAR)
end

local function hasCompletedBankStander()
	return Inventory:GetItemAmount(ITEM_IDS.BANK_STANDER_COMPLETE) > 0
end

local function isCorrectCategorySelected()
	local vb1169 = API.VB_FindPSettinOrder(VB_SETTINGS.CATEGORY)
	return vb1169 and vb1169.state == VB_SETTINGS.CATEGORY_ID
end

local function isCorrectProductSelected()
	local vb1170 = API.VB_FindPSettinOrder(VB_SETTINGS.PRODUCT)
	return vb1170 and vb1170.state == VB_SETTINGS.BANK_STANDER_ID
end
...
local function loadBankPreset()
	...
	-- Check if we got adamant bars after loading preset
	if not hasAdamantBar() then
		if presetAttempts >= SETTINGS.MAX_PRESET_ATTEMPTS then
			...
			print("No adamant bar found, will retry preset loading...")
			return false
		end
	end
...
local function configureCategoryAndProduct()
	...
	-- Select Misc Support option
	API.DoAction_Interface(
		0xffffffff,
		0xffffffff,
		1,
		INTERFACE_IDS.MISC_SUPPORT_OPTION.component,
		INTERFACE_IDS.MISC_SUPPORT_OPTION.subcomponent,
		INTERFACE_IDS.MISC_SUPPORT_OPTION.option,
		API.OFF_ACT_GeneralInterface_route
	)
...
local function handleCompletionDialog()
	if hasCompletedBankStander() then
		print("Bank stander completed, pressing space to continue")
...
-- Main Script
print("Starting Bank Standers Manufacturing Script")
...
while API.Read_LoopyLoop() do
	if isPlayerReady() then
		...
		if not hasAdamantBar() then
			print("No adamant bar found, loading bank preset")
			loadBankPreset()
			goto continue
		end
