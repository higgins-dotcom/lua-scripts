...
Functions:
- extractPuzzleState() - Gets current puzzle state from puzzle varbits
...
-- The active puzzle is selected by player varp 6168 (slide_puzzle_struct).
-- Struct parameter 6892 is the bank selector: 0 = hard, 1 = elite, 2 = extra_1.
local ACTIVE_PUZZLE_STRUCT_VARP = 6168
...
local PUZZLE_BANKS = {
	...
	[2] = { name = "extra_1", player_piece_base = 39380 },
}

-- The struct type is a cache parameter, so the Lua API only exposes the
-- selected struct ID through varp 6168. Keep the current cache's mapping
-- here instead of relying on the persistent *_started flags.
local PUZZLE_STRUCT_TYPES = {
	-- Third bank (extra_1; used by master-style puzzle content as well as
	-- other slide-puzzle content in the client cache).
	[33201] = 2,
	[33202] = 2,
	[35897] = 2,
	[38762] = 2,
...
local function readVarbit(varbitId)
	local success, value = pcall(function()
		return API.GetVarbitValue(varbitId)
	end)
...
local function readVarp(varpId)
	local success, setting = pcall(function()
		return API.VB_FindPSettinOrder(varpId, -1)
	end)
	if not success or not setting or type(setting.state) ~= "number" then
		return nil
	end
	return setting.state
end

local function isValidPuzzleState(state)
	if type(state) ~= "table" or #state ~= 25 then
		return false
	end

	local seen = {}
	for index = 1, 25 do
		local tile = state[index]
		if type(tile) ~= "number" or tile ~= math.floor(tile) or tile < 0 or tile > 24 or seen[tile] then
			return false
		end
		seen[tile] = true
	end
...
local function readPieceRange(firstVarbit)
	local state = {}
	for index = 0, 24 do
		local tile = readVarbit(firstVarbit + index)
		if tile == nil then
			return nil
		end
		state[index + 1] = tile
	end
	if not isValidPuzzleState(state) then
		return nil
	end
	return state
end
...
local function resolveActivePuzzleBank()
	local activeStructId = readVarp(ACTIVE_PUZZLE_STRUCT_VARP)
	local structType = activeStructId and PUZZLE_STRUCT_TYPES[activeStructId]
	local bank = structType and PUZZLE_BANKS[structType]
	if not bank then
		print("ERROR: Could not resolve active slide puzzle struct", activeStructId or "nil", "using parameter", PUZZLE_STRUCT_TYPE_PARAM)
		return nil, activeStructId
	end
	return bank, activeStructId
end

-- Return the currently selected puzzle bank and struct ID for diagnostics or
-- callers that need to distinguish hard, elite, and extra_1 puzzles.
function PuzzleModule.getActivePuzzleType()
	local bank, activeStructId = resolveActivePuzzleBank()
	if not bank then
		return nil, activeStructId
	end
	return bank.name, activeStructId
end

-- Extract the displayed puzzle from varbits. The active struct identifies the
-- bank; the client mirror is used only as a varbit-backed consistency source.
function PuzzleModule.extractPuzzleState()
	print("DEBUG: Starting varbit puzzle state extraction...")

	if not GetInterfaceOpenBySize(1931) then
		print("Could not extract puzzle state - slide puzzle interface is not open")
		return nil
	end
	...
	print("DEBUG: Active slide puzzle struct:", activeStructId, "bank:", bank.name)
	local playerState = readPieceRange(bank.player_piece_base)
	local clientState = readPieceRange(CLIENT_PIECE_VARBIT_BASE)

	if playerState and clientState and not statesEqual(playerState, clientState) then
		print("WARNING: Player and client puzzle varbits differ; refusing ambiguous state")
		return nil
	end

	local state = playerState or clientState
	if not state then
		print("ERROR: Could not read a valid 25-tile puzzle state from varbits")
		return nil
	end

	print("Extracted", bank.name, "puzzle state:", table.concat(state, ", "))
	return state
end

-- Varbit-only fallback for a short player/client synchronization window. The
-- client mirror is accepted only when the selected player bank is unavailable;
-- a valid-but-different player bank is treated as an ambiguous read.
function PuzzleModule.extractPuzzleStateFallback()
	print("DEBUG: Trying client puzzle varbit fallback...")
	if not GetInterfaceOpenBySize(1931) then
		print("Could not use fallback - slide puzzle interface is not open")
		return nil
	end

	local bank = resolveActivePuzzleBank()
	if bank and readPieceRange(bank.player_piece_base) then
		print("ERROR: Selected player puzzle bank is valid but client state is unavailable or ambiguous")
		return nil
	end

	local clientState = readPieceRange(CLIENT_PIECE_VARBIT_BASE)
	if clientState then
		print("DEBUG: Using validated client puzzle state:", table.concat(clientState, ", "))
		return clientState
	end
	print("ERROR: Client puzzle varbits do not contain a valid state")
	return nil
end
